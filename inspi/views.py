...
class ActivityOfTheWeekFilter(FilterSet):
...
class ActivityViewSet(viewsets.ModelViewSet):
    ...
    serializer_class = inspi_serializers.ActivityItemSerializer
...
class ExperimentItemViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            score = data['score']
            if score == 0:
                total_ratings = inspi_models.ExperimentItem.objects.filter(activity=activity).count()
                unclear_ratings = inspi_models.ExperimentItem.objects.filter(activity=activity).filter(score=0).count()
                if total_ratings >= 1:
                    unclear_rate = unclear_ratings / total_ratings
                    if unclear_rate >= 0.3:
                        activity = inspi_models.Activity.objects.filter(id=activity)
                        activity.update(is_public=False)
                        inspi_models.Activity.objects.get(id=activity).tags.add(inspi_models.Tag.objects.get(id=70))

        return super().create(request, *args, **kwargs)


class RandomActivityViewSet(viewsets.ModelViewSet):
    queryset = inspi_models.Activity.objects.filter(is_public=True).order_by("?")[:10]
    serializer_class = inspi_serializers.ActivitySerializer


class AdminActivityViewSet(viewsets.ModelViewSet):
    queryset = inspi_models.Activity.objects.order_by('-created_at').all()
    serializer_class = inspi_serializers.ActivityAdminSerializer


class ActivitySitemapViewSet(viewsets.ModelViewSet):
    queryset = inspi_models.Activity.objects.all()
    serializer_class = inspi_serializers.ActivitySitemapSerializer


class ActivityTimestampViewSet(viewsets.ModelViewSet):
    queryset = inspi_models.Activity.objects.filter(is_public=True).order_by('created_at')
    serializer_class = inspi_serializers.ActivityTimestampSerializer
...
class ChangePublishStatusViewSet(mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        activity_id = self.request.data.get('activity', None)

        if activity_id:
            activity: inspi_models.Activity = get_object_or_404(inspi_models.Activity, id=activity_id)
            activity.is_public = set_active
            activity.save()
            worked = True
...
class ActivityOfTheWeekViewSet(viewsets.ModelViewSet):
    queryset = inspi_models.ActivityOfTheWeek.objects.all()
    serializer_class = inspi_serializers.ActivityOfTheWeekSerializer
    filterset_class = ActivityOfTheWeekFilter
    filter_backends = (DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter)
...
class MaterialItemsViewSet(mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response({"material_activity_ids": return_array}, status=status.HTTP_200_OK)
